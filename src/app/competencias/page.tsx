      ...
      {items.map((competition) => (
        <article className="competition-card" key={competition.id}>
          <div className="competition-card-top">
            <span className={`competition-status ${competition.status}`}>{statusLabels[competition.status]}</span>
            <strong>{competition.year || "Año pendiente"}</strong>
          </div>
          <h2>{competition.name}</h2>
          <p className="competition-place">⌖ {[competition.city, competition.country].filter(Boolean).join(", ") || competition.venue || "Lugar por definir"}</p>
          <dl>
          ...
          {competition.notes && <p className="competition-note">{competition.notes}</p>}
          <small>Fuente: {competition.source_name || "Registro interno"}</small>
        </article>
      ))}
    </div>
  );
    ...
    <main className="module-page">
      <header className="module-header">
        <div>
          ...
          <p className="eyebrow">Calendario deportivo</p>
          <h1>Competencias</h1>
      ...
      <section className="module-content">
        {error ? <div className="error-banner">No pudimos cargar las competencias.</div> : (
          <>
            <div className="section-heading"><div><span className="section-kicker">Próximas</span><h2>{upcoming.length} competencias y eventos</h2></div></div>
            {renderCards(upcoming)}
            {past.length > 0 && <><div className="section-heading competition-past-heading"><div><span className="section-kicker">Histórico</span><h2>Eventos anteriores</h2></div></div>{renderCards(past)}</>}
          </>
        )}
      </section>
    </main>
